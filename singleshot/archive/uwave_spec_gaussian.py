...
def fit_gauss2D(image):
    ...
    initial_guess = moments(
        image
    )  # [amplitude, mux, muy, sigmax, sigmay, rotation, offset]
    # print(initial_guess)
    try:
...
def gauss2D(x, amplitude, mux, muy, sigmax, sigmay, rotation, offset):
    ...
    X = x[0]
    Y = x[1]
    A = (np.cos(rotation) ** 2) / (2 * sigmax**2) + (np.sin(rotation) ** 2) / (
        2 * sigmay**2
    )
    B = (np.sin(rotation * 2)) / (4 * sigmay**2) - (np.sin(2 * rotation)) / (
        4 * sigmax**2
    )
    C = (np.sin(rotation) ** 2) / (2 * sigmax**2) + (np.cos(rotation) ** 2) / (
        2 * sigmay**2
    )
    G = (
        amplitude
