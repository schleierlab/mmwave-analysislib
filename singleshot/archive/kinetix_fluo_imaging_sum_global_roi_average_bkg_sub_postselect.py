...
save_image = False
image_folder = 'C:\\tweezer_images_after_rearrange'
...
with h5py.File(h5_path, mode='r+') as f:
    ...
    row_indx = str(f['/globals/Tweezers'].attrs['TW_post_select_img_row_index'])
...
if run_number % 1 == 0: # all run number case
    ...
    roi_sig_1 = sub_image1[:, roi_x[0]:roi_x[1]]
    roi_bkg_1 = sub_image1[:, roi_x_bkg[0]:roi_x_bkg[1]]

    roi_sig_2 = sub_image2[:, roi_x[0]:roi_x[1]]
    ...
    roi_number_lst_1 = []
    for i in np.arange(site_roi_x.shape[0]):
        site_roi_signal = roi_sig_1[site_roi_y[i,0]:site_roi_y[i,1], site_roi_x[i,0]:site_roi_x[i,1]]
    ...
    atom_site_lst = []
    for i in np.arange(site_roi_x.shape[0]):
        site_roi_signal = roi_sig_2[site_roi_y[i,0]:site_roi_y[i,1], site_roi_x[i,0]:site_roi_x[i,1]]
        ...
        if site_roi_signal.sum() > threshold:
            ...
            atom_site_lst.append(i)
        else:
            atom_exist_lst_2.append(0)

    #print(atom_exist_lst_2)
    print('atom site list = ', atom_site_lst)

    if target_array == atom_site_lst:
        print("*************rearrange success!*************")
        if save_image == True:
            np.save(image_folder +'\\'+ row_indx + '.npy', sub_image2)
    ...
    pos = ax_mot_roi_1.imshow(roi_sig_1, **roi_img_color_kw)
    if show_site_roi:
    ...
    pos = ax_mot_roi_2.imshow(roi_sig_2, **roi_img_color_kw)
    if show_site_roi:
