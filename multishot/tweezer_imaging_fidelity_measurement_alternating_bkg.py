...
def avg_all_bkg_shots(folder, shots = 'defult', loop = True):
...
def avg_shots_multi_roi_avg_bkg_sub(folder, site_roi_y, site_roi_x, avg_bkg_img, shots = 'default', loop = True, plot_single_shots = False, image_scale = 'default'):
    n_shots = np.size([i for i in os.listdir(folder) if i.endswith('.h5')])
...
def histagram_fit_and_threshold(roi_number_lst, site_roi_x, plot_histagram = False, plot_double_gaussian_fit = False, plot_gaussian_poisson_fit = False, sub_bkg = False, cpa = 'default', print_value = False):
    ...
    #double_gaussian_fit
    # cpa = 4000 #np.max(second_shot_roi_number)/2 #1200 #2000
    # sigma1 = 0.1*cpa #0.1*cpa
    # sigma2 = 0.1*cpa #0.1*cpa
    # mu1 = 500
    # mu2 = cpa
...
### include the bkg region of site roi
...
avg_bkg_img, N = avg_all_bkg_shots(folder)

(data, roi_number_lst, N) = avg_shots_multi_roi_avg_bkg_sub(folder, site_roi_y_new, site_roi_x_new, avg_bkg_img)
