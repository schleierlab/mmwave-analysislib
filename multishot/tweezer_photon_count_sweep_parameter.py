...
def avg_survival_rate_sweep(roi_number_lst, th, site_roi_x, para, do_neighbour_bkg_sub=True):
    """
    ...
    fig.suptitle(f'n average = {n_average}')
    (ax1, ax2), (ax3, ax4), (ax5, ax6) = axs
    ax1.plot(para, survival_rate_each_para,'-o')
    ax1.grid()
    # ax1.set_xlabel('x [px]')
    ax1.set_ylabel(f'survival rate')
    ax1.set_title(f'average survival rate: {np.mean(survival_rate_each_para)}')

    ax2.plot(para, appear_rate_each_para,'-o')
    ax2.grid()
    # ax2.set_xlabel('x [px]')
    ax2.set_ylabel(f'appear rate')
    ax2.set_title(f'average appear rate: {np.mean(appear_rate_each_para)}')

    ax3.plot(para, lost_rate_each_para,'-o')
    ax3.grid()
    # ax3.set_xlabel('x [px]')
    ax3.set_ylabel(f'lost rate')
    ax3.set_title(f'average lost rate: {np.mean(lost_rate_each_para)}')

    ax4.plot(para, fidelity_each_para,'-o')
    ax4.grid()
    # ax4.set_xlabel('x [px]')
    ax4.set_ylabel(f'fidelity')
    ax4.set_title(f'average fidelity: {np.mean(fidelity_each_para)}')

    ax5.plot(para, loading_rate_each_para,'-o')
    ax5.grid()
    # ax4.set_xlabel('x [px]')
    ax5.set_ylabel(f'loading rate')
    ax5.set_title(f'average loading rate: {np.mean(loading_rate_each_para)}')

    ax6.plot(para, first_shot_count_sum_each_para,'-o')
    ax6.grid()
    # ax4.set_xlabel('x [px]')
    ax6.set_ylabel(f'count sum')
    ax6.set_title(f'average count sum: {np.mean(first_shot_count_sum_each_para)}')


def avg_survival_rate_sweep_indvidual_sites(roi_number_lst, th, site_roi_x, para, do_neighbour_bkg_sub=True):
    """
    Calculates the survival rate, appear rate, lost rate, and fidelity for each ROI in a given region of interest (ROI).
...
def photon_count_sweep(roi_number_lst, site_roi_x, para):
    """
    Calculates the survival rate, appear rate, lost rate, and fidelity for each ROI in a given region of interest (ROI).
    ...
    # first_shot_count_sum_each_para = np.array([np.sum(first_shot_atom_number[:,i::para.shape[0]]) for i in range(para.shape[0])])
    first_shot_count_sum_each_para = np.array([np.max(first_shot_atom_number[:,i::para.shape[0]]) for i in range(para.shape[0])])
...
def photon_count_sweep_individual_sites(roi_number_lst, site_roi_x, para):
    """
    Calculates the survival rate, appear rate, lost rate, and fidelity for each ROI in a given region of interest (ROI).
    ...
    first_shot_count_sum_each_para = np.array([[np.sum(first_shot_atom_number[j,i::para.shape[0]]) for i in range(para.shape[0])]for j in np.arange(tweezer_num)])

    first_shot_count_each_para = np.array([[first_shot_atom_number[j,i::para.shape[0]] for i in range(para.shape[0])]for j in np.arange(tweezer_num)])

    #print(first_shot_count_each_para.shape)

    n_repeat = first_shot_count_each_para.shape[2]
    # print(first_shot_atom_sum_each_para)
    ...
    for i in np.arange(tweezer_num):
        for j in np.arange(n_repeat):
            ax6.plot(para, first_shot_count_each_para[i,:, j],'o')
    ax6.grid()
    # ax4.set_xlabel('x [px]')
    ax6.set_ylabel(f'count sum')
    ax6.set_title(f'average count sum: {np.mean(first_shot_count_sum_each_para)}')
...
photon_count_sweep(roi_number_lst, site_roi_x, para)
# photon_count_sweep_individual_sites(roi_number_lst, site_roi_x, para)
