...
def rearrangement_success_rate(roi_number_lst, thold, site_roi_x, site_roi_y, n_shots, target_array, shots = 'defult',loop = False, plot = True): #thold means threshold
    ...
    success_number_lst = []
    success_arr = np.zeros((n, shots)) # success rearrange for each site, each shot
    fail_arr = np.zeros((n, shots))
    ...
    print('average success rate for each site:',np.mean(site_success_rate_lst))
    ...
    if plot == True:
        ...
        axs.set_title(f'{n_target} sites target array, {len(rearrange_shot)} rearrange shots')
        axs.set_xticks(unique_elements) # Adjust the x-axis ticks to be at the center of the bars
        plt.show()

        # site success rate
        fig, axs = plt.subplots(nrows=1, ncols=1)
        axs.plot(x_arr, site_success_rate,'o')
        axs.grid()
        axs.set_xlabel('x [px]')
        axs.set_ylabel(f'rearrangement success rate, {n_shots} shots')
        # axs.set_title(f'avg success rate: {np.mean(site_success_rate_lst)}')
        plt.show()

def rearrangement_debug(roi_number_lst, thold, site_roi_x, site_roi_y, n_shots, target_array, shots = 'defult',loop = False, plot = True): #thold means threshold
    # run rearrangement code,turn off the unloaded & unused sites in 40 sites but don’t do rearrangement, see what the survival rate is
    target_array = np.array(target_array)
    ...
    for cnt in rearrange_shot:
        target_indx = []
        target_surv = []
        for i in np.arange(n): # for the first len(target_array) sites, see what the survival rate is
            if surv_arr[i, cnt] + lost_arr[i, cnt] == 1: # atoms loaded in the first shot
                target_indx.append(i)
                target_surv.append(surv_arr[i, cnt])
            if len(target_indx) == n_target:
                break
        target_indx_lst.append(target_indx)
        target_surv_lst.append(target_surv)

    counter_1 = [0]*n
    counter_total = [0]*n
    for sublist_1, sublist_2 in zip(target_indx_lst, target_surv_lst):
        for num, value in zip(sublist_1, sublist_2):
            if value == 1:
                counter_1[num] += 1
            # Count every occurrence of num (either 1 or 0)
            counter_total[num] += 1
    ...
    x_arr = np.sum(n_site_roi_x, axis = 1)/2
    for i in range(n):
        if counter_total[i] > 0:
            ratio = counter_1[i] / counter_total[i]
            surv_lst.append(ratio)
            surv_indx_lst.append(x_arr[i])
        # else:
        #     ratio = 0

    atom_number_lst = []
    for i, sublist in enumerate(target_surv_lst):
        atom_number_lst.append(sublist.count(1))


    site_surv_rate_avg = np.mean(np.array(surv_lst))
    print('rearrangement shots ratio:', len(rearrange_shot)/shots)
    print('target rearrange sites survival rate avg:',site_surv_rate_avg)
    # print('average survival rate in rearrangement:',np.sum(surv_rate_arr)/len(surv_rate_arr))
    # print('average new appear rate in rearrangement:',np.sum(appear_rate_arr)/len(appear_rate_arr))
    ...
    if plot == True:
        ...
        # Calculate the frequency of each unique element
        unique_elements, counts = np.unique(atom_number_lst, return_counts=True)
        fig, axs = plt.subplots(nrows=1, ncols=1)
        axs.bar(unique_elements, counts, width=0.5) # Create a bar plot
        axs.grid(axis = 'y')
        axs.set_xlabel('Number of sites after "rearrangement"')
        ...
        # site success rate
        fig, axs = plt.subplots(nrows=1, ncols=1)
        axs.plot(surv_indx_lst, surv_lst,'o')
        axs.grid()
        axs.set_xlabel('x [px]')
        axs.set_ylabel(f'survival rate, {len(rearrange_shot)} shots average')
        axs.set_title(f'site survival rate avg: {site_surv_rate_avg:.3f}')
        plt.show()
...
rearrangement_debug(roi_number_lst, th, site_roi_x, site_roi_y, n_shots, target_array, shots = 'defult',loop = False, plot = True)
